...
const create = Account => async (oidc, req, res, next) => {
  ...
  try {
    ...
    if (prompt.name !== 'create') return next('could not create account')
...
const errorHandler = (err, res) => {
  if (!res.finished) {
    ...
    return res.end(JSON.stringify({
      ...(err.error ? { error: err.error } : { error: err }),
  ...
  for (let n = 0; n < torun.length; n++) {
    torun[n](req, res, function (e, r) {
      ...
      if (n >= torun.length - 1) {
        ...
        return action(oidc, req, res, localnext)
          .then(() => successHandler(res), (ferr) => errorHandler(ferr, res))
      }
    })
  }
}
